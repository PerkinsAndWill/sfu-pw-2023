...
export function raycasting(
  ...
  finSpacing
    ...
    console.log(ceiling, floor, wall, glazing);
    if (!finSpacing) {
      console.log(
        `Skipping iteration ${simNumber} because fin spacing is null`
      );
    } else {
      let shadingFactors = [];
        ...
        ceiling.length === 1
          ...
          : new Mesh(
              BufferGeometryUtils.mergeGeometries(
                ceiling.map((x) => x.geometry)
              )
            ),
        floor.length === 1
          ? floor[0]
          : new Mesh(
              BufferGeometryUtils.mergeGeometries(floor.map((x) => x.geometry))
            ),
        wall.length === 1
          ? wall[0]
          : new Mesh(
              BufferGeometryUtils.mergeGeometries(wall.map((x) => x.geometry))
            ),
        glazing.length === 1
          ? glazing[0]
          : new Mesh(
              BufferGeometryUtils.mergeGeometries(
                glazing.map((x) => x.geometry)
              )
            ),
      ];
      ...
      sensorsPos.forEach((sensorPos) => {
        ...
        for (let i = 0; i < shading.length; ++i) {
          for (let j = 0; j < rays.length; ++j) {
            ...
            let intersections = [];
            //let result = new Vector3();
            //console.log(shading[i], i);
            //let bb = shading[i].geometry.boundingBox;
            //if(!bb || bb && raycaster.ray.intersectBox(bb, result) !== null)
            {
