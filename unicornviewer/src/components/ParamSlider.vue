<template>
  ...
  >
    ...
    <v-slider
      ...
      @end="() => {preEnd(); updateBackendData(modelName, model_internal);}"
    >
      <template #append>
        <v-text-field
          v-model="model_internal"
    ...
    export default {
        ...
        methods: {
            updateBackendData(key, value) {
                console.log("paramSlider", key, value, window.Interop)
                if (window.Interop) {
                    window.Interop.updateBackendData(key, value)
                }
            },
        }
