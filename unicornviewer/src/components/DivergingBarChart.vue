<template>
  <div :id="'divergingbar_' + title" />
</template>

<script>
import * as d3 from "d3";

export default {
  name: "DivergingBarChart",
  props: {
    data: {
      type: Array,
      required: true,
    },
    title: {
      type: String,
      required: true
    }
  },
  data() {
    return {
      colorScale: d3.scaleOrdinal(d3.schemeCategory10),
      svgChart: null,
    };
  },
  watch: {
    data: {
      handler(newVal, oldVal) {
      
        // watch it
        this.createChart(this.data);
      },
      deep: true,
    },
  },
  mounted() {
    this.createChart(this.data);
  },
  methods: {
    createChart(data) {
      console.log("creating divering", this.data)
      d3.select("#divergingbar_" + this.title).select("svg").remove();

      // Specify the chart’s dimensions.
      const barHeight = 25;
      const marginTop = 30;
      const marginRight = 60;
      const marginBottom = 10;
      const marginLeft = 60;
      const height =
        Math.ceil((data.length + 0.1) * barHeight) + marginTop + marginBottom;
      const width = 300;

      // Create the positional scales.
      const x = d3
        .scaleLinear()
        .domain([-1,1])
        .rangeRound([marginLeft, width - marginRight]);

      const y = d3
        .scaleBand()
        .domain(data.map((d) => d.title))
        .rangeRound([marginTop, height - marginBottom])
        .padding(0.1);

      // Create the format function.

      // Create the SVG container.
      const svg = d3
        .select("#divergingbar_" + this.title)
        .append("svg")
        .attr("viewBox", [0, 0, width, height])
        .attr("width", width)
        .attr("height", height)
        .attr(
          "style",
          "max-width: " + width + "px; height: auto; font: 10px sans-serif;"
        );

      // Add a rect for each state.
      svg
        .append("g")
        .selectAll()
        .data(data)
        .join("rect")
        .attr("fill", (d) => d3.schemeRdBu[3][d.value > 0 ? 2 : 0])
        .attr("x", (d) => x(Math.min(d.value, 0)))
        .attr("y", (d) => y(d.title))
        .attr("width", (d) => Math.abs(x(d.value) - x(0)))
        .attr("height", y.bandwidth());

      // Add a text label for each state.
      svg
        .append("g")
        .attr("font-family", "sans-serif")
        .attr("font-size", 10)
        .selectAll()
        .data(data)
        .join("text")
        .attr("text-anchor", (d) => (d.value < 0 ? "end" : "start"))
        .attr("x", (d) => x(d.value) + Math.sign(d.value - 0) * 4)
        .attr("y", (d) => y(d.title) + y.bandwidth() / 2)
        .attr("dy", "0.35em")
        .text((d) => d.value);

      // Add the axes and grid lines.
      svg
        .append("g")
        .attr("transform", `translate(0,${marginTop})`)
        .call(d3.axisTop(x).ticks(width / 80))
        .call((g) =>
          g
            .selectAll(".tick line")
            .clone()
            .attr("y2", height - marginTop - marginBottom)
            .attr("stroke-opacity", 0.1)
        )
        .call((g) => g.select(".domain").remove());

        svg.append("g")
        .append("text")
        .attr("x",  width/2 )
        .attr("y", 0)
        .attr("dy", ".71em")
        .style("font-size", "larger")
        .style("text-anchor", "middle")
        .text(this.title);

      svg
        .append("g")
        .attr("transform", `translate(${x(0)},0)`)
        .call(d3.axisLeft(y).tickSize(0).tickPadding(6))
        .call((g) =>
          g
            .selectAll(".tick text")
            .filter((d, i) => data[i].value < 0)
            .attr("text-anchor", "start")
            .attr("x", 6)
        );
    },
  },
};
</script>
<style scoped></style>
